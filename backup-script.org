...
#+begin_src sh :eval never :tangle restore.sh
sudo rm /etc/X11/xinit/xinitrd.c/40-libcanberra-gtk-module.sh
#+end_src

*** WAIT Restore pacman cache
*** Install Pacman Packages
#+begin_src sh :eval never :tangle restore.sh
sudo pacman -Syu --needed --answerclean y --answerdiff n --answerupgrade y --noremovemake --cleanafter - < pacman-packages.txt
#+end_src
...
#+begin_src sh :eval never :tangle restore.sh
yay -Syu --needed --answerclean y --answerdiff n --answerupgrade y --noremovemake --cleanafter - < aur-packages.txt
#+end_src
** Restore Home Folder
